...
from .schemas import (
    ...
    TemplateCreate,
    TemplateImportCommit,
    TemplateUpdate,
)
from .template_import import MAX_IMPORT_BYTES
...
_MESSAGES = {
    ...
    "duplicate_tiktok_account": "检测到重复的 TikTok 账号。",
    "tiktok_login_required": "TikTok 账号尚未登录。",
    "tiktok_identity_unavailable": "无法读取 TikTok 账号身份。",
    "tiktok_identity_changed": "TikTok 账号身份已发生变化。",
    ...
    "template_unavailable": "所选评论树已停用或删除。",
    "unsupported_import_type": "仅支持 .xlsx 文件。",
    "import_file_invalid": "导入文件无效或已损坏。",
    "import_file_too_large": "导入文件不能超过 2 MiB 或 5000 数据行。",
    "import_tree_failed": "评论树导入失败。",
...
def create_comment_campaign_blueprint(service_or_factory: object) -> Blueprint:
    ...
    def enable_template(template_id: str):
        payload = _parse(ExpectedRevision)
        return _data(
            _call(service(), "enable_template", template_id, payload.expected_revision)
        )

    @blueprint.post("/comment-templates/<template_id>/delete")
    def delete_template(template_id: str):
        payload = _parse(ExpectedRevision)
        return _data(
            _call(service(), "delete_template", template_id, payload.expected_revision)
        )
    ...
    def preview_template_import():
        ...
        return _data(
            _call(service(), "preview_template_import", upload.filename or "", content)
        )
    ...
    def list_profile_metadata():
        return _profile_envelope(_call(service(), "list_profile_metadata"))

    @blueprint.post("/comment-profile-metadata/sync")
    def sync_profile_metadata():
        _parse(EmptyRequest)
        return _profile_sync_envelope(service)
    ...
    @blueprint.post("/comment-profile-selection/preview")
    def preview_profile_selection():
        payload = _parse(ProfileSelectionPreview)
        return _data(_call(service(), "preview_profile_selection", payload))
    ...
    def handle_upload_too_large(error: RequestEntityTooLarge):
        del error
        return _error("import_file_too_large"), 413
    ...
    def handle_error(error: Exception):
        ...
        details = error.details if isinstance(error, AllocationError) else None
        return _error(code, details), status
...
def _profile_envelope(value: Any):
    safe = _redact(value)
    return jsonify({"data": safe["data"], "meta": safe["meta"]}), 200


def _profile_sync_envelope(service_factory: Callable[[], object]):
    try:
        return _profile_envelope(_call(service_factory(), "sync_profile_metadata"))
    except (ValueError, TypeError, ValidationError):
        raise RuntimeError("comment profile sync failed") from None


def _error(code: str, details: Mapping[str, Any] | None = None):
    safe_code = code if code in _MESSAGES else "internal_error"
    body: dict[str, Any] = {"code": safe_code, "message": _MESSAGES[safe_code]}
    if safe_code == "allocation_unsatisfied" and details:
        projected: dict[str, Any] = {}
        reason = details.get("reason")
        if isinstance(reason, str) and reason in ALLOCATION_REASONS:
            projected["reason"] = reason
        for name in ("required_count", "eligible_count"):
            value = details.get(name)
            if type(value) is int and value >= 0:
                projected[name] = value
        profiles = details.get("display_profiles")
        if (
            isinstance(profiles, (list, tuple))
            and len(profiles) <= 2
            and all(isinstance(value, str) for value in profiles)
        ):
            projected["display_profiles"] = list(profiles)
        if projected:
            body["details"] = _redact(projected)
...
def _error_status(error: Exception) -> tuple[int, str]:
    ...
    if isinstance(error, CampaignError):
        if error.code == "import_file_too_large":
            return 413, error.code
        if error.code in _UNAVAILABLE_CODES:
            return 503, error.code
        if error.code in {
            "revision_conflict", "approval_revision_mismatch", "template_unavailable"
        }:
...
def _forbidden_string(value: str) -> bool:
    """Drop transport credentials from diagnostic strings, not ordinary comments."""
    normalized = value.casefold()
    return (
        "ws://" in normalized
        or "wss://" in normalized
        or "raw_adspower" in normalized
        or "raw-adspower" in normalized
        or "cookie=" in normalized
        or "set-cookie:" in normalized
        or "authorization:" in normalized
        or "api_key=" in normalized
