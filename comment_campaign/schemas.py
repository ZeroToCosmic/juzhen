...
class ImportedTreeNode(_StrictInput):
    """Raw commit node only; derived preview fields are never client input."""

    node_no: str = Field(max_length=500)
    parent_node_no: str | None = Field(default=None, max_length=500)
    text: str = Field(max_length=10_000)


class ImportedTree(_StrictInput):
    """Defensive bounds intentionally exceed one-tree business limits."""

    name: str = Field(max_length=1_000)
    nodes: list[ImportedTreeNode] = Field(max_length=5_000)
...
class TemplateImportCommit(_StrictInput):
    ...
    @model_validator(mode="after")
    def validate_total_nodes(self) -> "TemplateImportCommit":
        if sum(len(tree.nodes) for tree in self.trees) > 5_000:
            raise ValueError("import cannot exceed 5000 nodes")
        return self
