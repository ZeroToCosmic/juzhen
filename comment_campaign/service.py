...
from .allocation import AllocationError, allocate, validate_template_tree
...
class CommentCampaignService:
    ...
    def prepare_campaign(self, campaign_id: str) -> dict:
        ...
        if not eligible:
            return {"prepared": (), "failed": (), "close_confirmed": True}
        result = asyncio.run(self._executor.prepare_batch(campaign_id, eligible[:campaign["batch_size"]]))
        return {"prepared": result.prepared, "failed": result.failed, "close_confirmed": result.close_confirmed}

    async def job_prepare_campaign(self, campaign_id: str) -> dict:
        ...
        if campaign["status"] not in {"queued", "running"}:
            return {"prepared": (), "failed": (), "close_confirmed": True}
        if campaign["status"] == "queued":
            campaign = self.store.transition_campaign_status(campaign_id, campaign["revision"], "running")
        eligible = self.store.eligible_assignment_ids(campaign_id)
        if not eligible:
            return {"prepared": (), "failed": (), "close_confirmed": True}
        result = await self._executor.prepare_batch(campaign_id, eligible[:campaign["batch_size"]])
        return {"prepared": result.prepared, "failed": result.failed, "close_confirmed": result.close_confirmed}
    ...
    def enqueue_next_prepare_generation(self, campaign_id: str) -> dict:
        ...
        job = self._enqueue_prepare_generation(coordinator, campaign_id, generation)
    ...
    def list_profile_metadata(self) -> list[dict]:
        if self._profile_provider is not None:
            ...
            self.store.sync_profile_identities(profiles)
        return self.store.list_comment_profiles()
    ...
    def resume_verified_children(self, campaign_id: str, parent_assignment_id: str) -> list[str]:
        ...
        if changed and campaign and campaign.get("status") == "running" and self._queue_coordinator is not None:
            ...
            if callable(enqueue):
                generation = self.store.next_prepare_generation(campaign_id)
                self._enqueue_prepare_generation(self._queue_coordinator, campaign_id, generation)
        return changed
    ...
    def create_campaign(self, payload: CampaignCreate | dict[str, Any], campaign_id: str | None = None) -> dict:
        ...
        if any(
            self.store.get_profile_identity(profile_ref) is None
            for profile_ref in campaign.profile_refs
        ):
    ...
    def plan_campaign(self, campaign_id: str, seed: str | None = None, *, expected_revision: int | None = None) -> dict:
        ...
        if current_template is None or template is None or not current_template["enabled"]:
            raise CampaignValidationError("template_invalid")
        ...
        profile_snapshot = self._profile_snapshot(campaign["profile_refs"], eligibility_at)
    ...
    def approve_campaign(self, campaign_id: str, expected_revision: int) -> dict:
        ...
        try:
            if campaign["start_mode"] == "scheduled":
                ...
                job = coordinator.enqueue_at(campaign_id, scheduled_at)
            else:
                generated = getattr(coordinator, "enqueue_prepare_generation", None)
                generation = self.store.next_prepare_generation(campaign_id)
                job = self._enqueue_prepare_generation(coordinator, campaign_id, generation) if callable(generated) else coordinator.enqueue_prepare(campaign_id)
        except Exception as exc:
    ...
    def _enqueue_prepare_generation(self, coordinator: Any, campaign_id: str, generation: int) -> Any:
        """Queue first, then durably mark that generation as safely accepted."""
        job = coordinator.enqueue_prepare_generation(campaign_id, generation)
    ...
    def _profile_snapshot(self, profile_refs: list[str], eligibility_at: datetime) -> list[dict]:
        snapshot: list[dict] = []
        for profile_ref in profile_refs:
            metadata = self.store.get_profile_metadata(profile_ref)
            identity = self.store.get_profile_identity(profile_ref)
            ...
            if cooldown_until:
                try:
                    ...
                    raise AllocationError() from exc
                if cooldown.tzinfo is None or cooldown > eligibility_at:
                    metadata = {**metadata, "enabled": False}
            snapshot.append({**metadata, "display_profile": identity["display_profile"]})
        return snapshot
