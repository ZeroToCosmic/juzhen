...
class CommentActionIdentityRecord(Base):
    __tablename__ = "comment_action_identities"
    __table_args__ = (
        CheckConstraint(_action_id_sql("action_id"), name="ck_comment_action_identity_id"),
    )
    action_id: Mapped[str] = mapped_column(String(30), primary_key=True)
    campaign_id: Mapped[str | None] = mapped_column(
        ForeignKey("comment_campaigns.id", ondelete="SET NULL"),
        nullable=True,
        unique=True,
    )
    source_revision: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    content_checksum: Mapped[str] = mapped_column(String(71), nullable=False, default="")
    tombstoned_at: Mapped[str | None] = mapped_column(String(40), nullable=True)
    created_at: Mapped[str] = mapped_column(String(40), nullable=False)


class CommentActionDebugRunRecord(Base):
    __tablename__ = "comment_action_debug_runs"
    __table_args__ = (
        UniqueConstraint("run_id", name="uq_comment_action_debug_run"),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    run_id: Mapped[str] = mapped_column(String(80), nullable=False)
    action_id: Mapped[str] = mapped_column(String(30), nullable=False)
...
class CommentActionReleaseRecord(Base):
    __tablename__ = "comment_action_releases"
    __table_args__ = (
        UniqueConstraint("action_id", "revision", name="uq_comment_action_release"),
        UniqueConstraint(
            "action_id",
            "source_revision",
            "content_checksum",
            name="uq_comment_action_source_release",
        ),
        UniqueConstraint("release_checksum", name="uq_comment_action_release_checksum"),
        CheckConstraint(
            _checksum_sql("content_checksum"),
            name="ck_comment_action_content_checksum_shape",
        ),
        CheckConstraint(
            _checksum_sql("release_checksum"),
            name="ck_comment_action_release_checksum_shape",
        ),
    )
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    action_id: Mapped[str] = mapped_column(String(30), nullable=False)
    ...
    debug_run_id: Mapped[str | None] = mapped_column(String(80), nullable=True)
    actor: Mapped[str] = mapped_column(String(120), nullable=False)
    waiver_reason: Mapped[str] = mapped_column(Text, nullable=False, default="")
    central_revision: Mapped[int | None] = mapped_column(Integer, nullable=True)
