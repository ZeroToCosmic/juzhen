...
from .identity import AccountObservation, normalize_tiktok_account_key
...
async def read_tiktok_identity(page: Any, account_definition: dict[str, Any], *, resolver: Any | None = None) -> AccountObservation:
    """Read exactly one current TikTok account control without trusting metadata."""
    try:
        selected = resolver or StrictLocatorResolver()
        resolved = await selected.resolve(page, account_definition, require_editable=False)
        handle = getattr(resolved, "handle", resolved)
        value = await handle.evaluate(
            "element => ({text: String(element.innerText || element.textContent || ''), "
            "href: String(element.href || ''), "
            "logged_in: element.getAttribute('data-logged-in') === 'false' ? false : undefined})"
        )
    except CampaignValidationError as exc:
        if exc.code == "tiktok_login_required":
            raise
        raise CampaignValidationError("tiktok_identity_unavailable") from None
    except Exception:
        raise CampaignValidationError("tiktok_identity_unavailable") from None
    if not isinstance(value, dict):
        raise CampaignValidationError("tiktok_identity_unavailable")
    if value.get("logged_in") is False or value.get("is_logged_in") is False or value.get("login_required") is True:
        raise CampaignValidationError("tiktok_login_required")

    text = value.get("text")
    href = value.get("href")
    if not isinstance(text, str) or not isinstance(href, str):
        raise CampaignValidationError("tiktok_identity_unavailable")
    visible_username = " ".join(text.split())
    if not visible_username:
        raise CampaignValidationError("tiktok_identity_unavailable")
    if href:
        account_key = _tiktok_account_key_from_href(href)
        try:
            visible_key = normalize_tiktok_account_key(visible_username)
        except CampaignValidationError:
            visible_key = None
        if visible_key is not None and visible_key != account_key:
            raise CampaignValidationError("tiktok_identity_unavailable")
        canonical_href = href
    else:
        account_key = normalize_tiktok_account_key(visible_username)
        canonical_href = None
...
def _tiktok_account_key_from_href(href: str) -> str:
    """Accept only a canonical TikTok profile URL; no redirect-like variants."""
    if not href or href != href.strip() or any(ord(char) < 32 for char in href):
        raise CampaignValidationError("tiktok_identity_unavailable")
    try:
    ...
    if (
        ...
        raise CampaignValidationError("tiktok_identity_unavailable")
    match = re.fullmatch(r"/@([A-Za-z0-9._]{1,24})", parsed.path)
    if match is None:
        raise CampaignValidationError("tiktok_identity_unavailable")
    return normalize_tiktok_account_key(match.group(1))


async def verify_logged_in_username(page: Any, expected_username: str, account_definition: dict[str, Any], *, resolver: Any | None = None) -> dict[str, str | None]:
    """Compatibility wrapper for callers that still carry an expected username."""
    evidence = await read_tiktok_identity(page, account_definition, resolver=resolver)
