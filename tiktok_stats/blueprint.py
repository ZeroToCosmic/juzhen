...
def create_tiktok_stats_blueprint() -> Blueprint:
    ...
    @blueprint.get("/api/tiktok-stats/videos")
    def videos():
        try:
            allowed = {
                "query", "account_id", "published_from", "published_to",
                "sort", "direction", "page", "page_size",
            }
            _reject_unknown_query(allowed)
            filters = {
                key: request.args[key]
                for key in ("query", "account_id", "published_from", "published_to")
                if key in request.args
            }
            result = _query_service().query_video_table(
                filters,
                request.args.get("sort", "last_collected_at"),
                request.args.get("direction", "desc"),
                _query_int("page", 1),
                _query_int("page_size", 50, maximum=_MAX_PAGE_SIZE),
            )
