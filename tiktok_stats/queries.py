...
_VIDEO_SORTS = {
    ...
    "last_collected_at": "p.last_seen_at",
}
_BASELINE_STATES = {
    "ready",
    "first_day",
...
class StatisticsQueryService:
    ...
    def query_video_table(
        ...
        unknown = sorted(
            set(filters) - {"query", "account_id", "published_from", "published_to"}
        )
        if unknown:
            raise ValueError(f"unknown video filter: {unknown[0]}")
        sort_sql = _VIDEO_SORTS.get(sort)
        if sort_sql is None:
        ...
        params: list[Any] = []
        raw_query = str(filters.get("query") or "").strip()
        if "/video/" in raw_query:
            raw_query = raw_query.split("/video/", 1)[1].split("?", 1)[0].split("/", 1)[0]
        elif raw_query.startswith("@"):
            raw_query = raw_query[1:]
        search = _search_pattern(raw_query)
        if search is not None:
            clauses.append(
                "(lower(COALESCE(p.description, '')) LIKE ? ESCAPE '\\' "
                "OR lower(p.video_id) LIKE ? ESCAPE '\\' "
                "OR lower(a.username) LIKE ? ESCAPE '\\')"
            )
            params.extend((search, search, search))
        if filters.get("account_id") not in (None, ""):
            raw_account_id = filters["account_id"]
            try:
                account_id = int(raw_account_id)
            except (TypeError, ValueError):
                raise ValueError("account_id must be a positive integer") from None
            if account_id < 1 or str(account_id) != str(raw_account_id):
        ...
        for key, operator in (("published_from", ">="), ("published_to", "<=")):
            ...
            if value in (None, ""):
                continue
            try:
                normalized_date = date.fromisoformat(str(value)).isoformat()
            except ValueError:
                raise ValueError(f"{key} must be an ISO date") from None
            published_dates[key] = normalized_date
            clauses.append(f"date(p.created_at) {operator} ?")
            params.append(normalized_date)
        if published_dates.get("published_from", "") > published_dates.get(
        ...
        summary_row = self.connection.execute(
            ...
            SELECT COUNT(*) AS video_count,
                   ...
                   COUNT(p.view_count) AS known_views,
                   COUNT(p.like_count) AS known_likes,
                   COUNT(p.comment_count) AS known_comments
        ...
        total = int(summary_row["video_count"])
        def complete_total(metric: str) -> int | None:
            if total == 0:
                return 0
            return (
                int(summary_row[f"total_{metric}"])
                if int(summary_row[f"known_{metric}"]) == total
                else None
            )
        rows = [
            {
            ...
            for row in self.connection.execute(
                f"""
                SELECT p.video_id, p.description, p.account_id, a.username,
                       p.created_at AS published_at, p.view_count AS views,
                       p.like_count AS likes, p.comment_count AS comments,
                       p.last_seen_at AS last_collected_at
                FROM posts_current p
                JOIN tracked_accounts a ON a.id = p.account_id
                WHERE {where_sql}
                ORDER BY ({sort_sql} IS NULL) ASC, {sort_sql} {direction_sql}, p.video_id ASC
                LIMIT ? OFFSET ?
                """,
        ...
        return {
            "summary": {
                "video_count": total,
                "total_views": complete_total("views"),
                "total_likes": complete_total("likes"),
                "total_comments": complete_total("comments"),
            },
            "rows": rows,
            "page": page,
            "page_size": page_size,
