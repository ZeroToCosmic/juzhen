...
本版本于 2026-08-17 经产品方批准进入实施规划。批准后任何产品行为、状态、安全边界或验收门槛变更必须发布后续 PRD 版本，不得直接回写 v3.2 规避变更审查。
...
5. v3.1、v2、旧实施蓝图、交接文档：历史参考，不具规范效力。

发现冲突时，开发必须暂停相关范围，先修订低优先级文档或代码。禁止通过口头约定绕过。

### 0.2 被替代文档

- `docs/PRD-业务控制系统.v3.1.md`：被本文件整体替代；保留为历史基线，不回写修订。
...
| `tasks` | task_id、customer_id、task_kind、strategy_revision、config_snapshot、missed_policy、missed_window_seconds、cancel_requested、bound_device_id、tree_generation、status、unresolved_count、revision |
...
| `recovery_ingest_items` | ingest_id、device_id、source_db_uuid、source_recovery_epoch、source_revision、payload_checksum、durable_ingest_ack_at、disposition、archive_ref |
...
- `recovery_ingest_items` 对 `(device_id, source_db_uuid, source_recovery_epoch, source_revision)` 唯一；同键 checksum 不同必须冲突，不得插入第二行。disposition 使用数据库 CHECK 枚举；`RETRYABLE_REJECTED` 可在后续成功摄取时 CAS 转为三个可清理终值之一，`APPLIED/DUPLICATE/QUARANTINED_DURABLE` 写入后不可互转。
...
依赖传播仅支持以下固定策略；v3.2 不开放客户自定义失败边策略：
...
1. Agent 以当前有效 Device Session 上传原始信封，保留 `source_device_id/source_db_uuid/source_recovery_epoch/source_revision/event_id/payload_checksum`。入口必须校验 source_device_id 等于当前 Session 绑定 device_id，且 source_db_uuid 属于该设备历史登记；不匹配返回 403 `RECOVERY_SOURCE_DEVICE_MISMATCH`，不写隔离库。唯一幂等键为 `(device_id, source_db_uuid, source_recovery_epoch, source_revision)`；同键同 checksum 返回原 durable ACK/disposition，同键不同 checksum 返回 `409 SOURCE_REVISION_CHECKSUM_CONFLICT`、不推进水位且 Agent 必须保留原记录。
...
- Web fallback 与 Agent 数据面使用独立请求 semaphore 和数据库连接池预算；至少 30% 数据库连接保留给 Agent 心跳、Device Session、Lease 和 Receipt，Web fallback 不得借用保留容量。
...
| `recovery_ingest.batch` | 当前 Device Session | source_device_id、source_db_uuid、source_recovery_epoch、起止 revision、最多 500 条且解压后 <=10 MiB 的原始信封 | source_device_id/数据库归属先与 Session 校验；每条返回 durable_ack、disposition 或 retryable error；整批部分成功必须逐条可重放，不得用批级 200 暗示全部成功 |
...
OpenAPI 错误码必须至少增加：`INSUFFICIENT_SINGLE_DEVICE_CAPACITY`、`CAPACITY_DIAGNOSTIC_UNAVAILABLE`、`DEVICE_PERMANENT_FAILURE_CONFLICT`、`DETERMINISTIC_EVIDENCE_GUARD_FAILED`、`UNRESOLVED_SEAL_GUARD_FAILED`、`LATE_RECONCILIATION_GUARD_FAILED`、`SOURCE_REVISION_CHECKSUM_CONFLICT`、`RECOVERY_SOURCE_DEVICE_MISMATCH`、`RECOVERY_INGEST_RETRYABLE_REJECTED` 和 `WEB_FALLBACK_RATE_LIMITED`。所有内部 details 继续受第 3.2 节隔离规则约束。
...
- Web fallback 与 Agent 控制数据面必须使用独立 admission/连接预算；Web 流量耗尽自身预算时不得排队占用 Agent 保留容量。
...
| `adspower_api_max_concurrency` | 1 | 每台 Agent 所有模块共享的 Local API 最大并发；v3.2 不允许调高 |
...
| `agent_db_pool_reserved_percent` | 30 | Agent 控制数据面保留的数据库连接比例 |
...
配置修改必须小步灰度并观察失败率、熔断率、DLQ、PUBLISHED_UNVERIFIED、CLOSED_UNVERIFIED 和资源水位。不得直接改库绕过版本化服务。标注“v3.2 不允许调高”的安全上限只能通过新版 PRD 与重新验收修改。
...
下列五项是从 v3.1 升级到 v3.2 的强制发布阻塞项，不是后续优化：

| 阻塞 ID | 修订 | 最早退出门 | 必须通过的验收 |
|---|---|---|---|
| RB-01 | 永久设备故障下按未知结果封存 | M5c | F-23、F-26、F-27 |
| RB-02 | 单机评论树过滤容量 fail-fast 与诊断 | M5c | F-24 |
