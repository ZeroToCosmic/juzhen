...
本文件是业务控制系统产品行为、系统边界、状态、安全、里程碑和验收要求的唯一权威 PRD。
...
| Agent | 设备身份、窗口观察、WAL、Receipt、工作单元执行、库存同步 | 决定客户权限、修改 Central 权威状态、跨设备协调 |
| Worker | 节点激活、清洗、验证、归档和回收 | 增删初始执行计划、旁路状态机或跳过 Fencing |
...
| 策略 revision/config_snapshot | Central | Agent 冻结快照 | 已创建任务永不读取本地草稿 |
...
| 原始 WAL/Receipt | Agent | Central 已接收副本 | ACK 后且 age >=7 天才可清理；未 ACK 数据无论年龄均不得删除 |
...
| `devices` | device_id、status、capabilities、agent_version；运营方全局唯一 |
...
| `tasks` | task_id、customer_id、task_kind、strategy_revision、config_snapshot、missed_policy、missed_window_seconds、cancel_requested、bound_device_id、tree_generation、status、revision |
| `subtasks` | subtask_id、task_id、不可变 customer_id、account_id、device_id、profile_id、reassign_blocked、status、revision |
| `attempts` | attempt_id、subtask_id、不可变 customer_id、attempt_no、recovery_epoch、generation、stage、side_effect_started、error_category、status |
...
| `comment_tree_reservations` | task_id 唯一、customer_id、bound_device_id、tree_generation、required_nodes、reserve_slots、status、revision |
| `comment_tree_accounts` | task_id、customer_id、account_id、bound_device_id、tree_generation、slot_kind、status；status=`RESERVED_SPARE/IN_USE/RELEASED/INVALID`，(task_id, account_id) 唯一 |
| `cleanup_jobs` | cleanup_id、profile_id、binding_revision、recovery_epoch、generation、status、next_attempt_at |
| `outbox/inbox` | 消息 ID 唯一；Outbox 与业务事务同库提交 |
| `audit_events` | actor、customer_id、resource、action、before/after、request_id、created_at |
| `system_state` | 当前 recovery_epoch、当前 web_session_epoch、调度冻结状态、恢复阶段；单行 CAS 更新 |
...
- `comment_campaign` 必须满足 bound_device_id 非空且 tree_generation=1；其他 task_kind 的两个字段必须为空。`comment_tree_accounts` 对 `status IN (RESERVED_SPARE, IN_USE)` 建立 account_id 部分唯一约束，禁止同一账号同时预留给多棵树。
...
- Task 创建时冻结 revision 与 config_snapshot。
- Agent 只执行 Work Unit 携带的冻结快照，不读取本地策略数据库决定生产行为。
...
3. Comment Campaign 在同一事务选择一台 Device，写入不可变树级绑定，并按第 8.3 节原子预留容量。
4. Central 在同一事务完成 DAG 拆解，写 Task、全部初始 SubTask、依赖、config_snapshot、评论树预留和 Outbox；Worker 不得增删初始执行计划。
...
Lease 到期或撤销必须由一个 PostgreSQL 事务收敛。`BOUND_DEVICE_PERMANENT_FAILURE` 事件优先按第 8.3 节处理，不进入下述通用重试分支；下述分支只处理 TTL 到期、临时离线、暂停和非永久撤销：
...
- 预留从 Task 创建保持至 Task 终态；终态聚合事务必须把全部 `RESERVED_SPARE/IN_USE` 行转 `RELEASED` 并释放容量预留。`INVALID` 行保留审计但不占活跃唯一约束。账号失效导致实际容量不足时只进入同机等待，不得修改 bound_device_id。
- 树内账号不重复；候选账号必须属于同一 customer_id。
- 新增备用账号仅检索当前 Device 的 ACTIVE、已绑窗、未参与本树账号；已预留换号只消费本树 `RESERVED_SPARE`。
...
- Device 永久故障的其他任何情况，包括任一 Attempt 已进入外部副作用、WAL 不完整或证据不确定：对应 Attempt/SubTask 转 `PUBLISHED_UNVERIFIED`，Task 转 `MANUAL_REVIEW` 并停止新分配；预留在人工收敛并进入 Task 终态时释放。
...
状态集：`PENDING`、`QUEUED`、`RUNNING`、`PAUSED`、`MANUAL_REVIEW`、`SUCCEEDED`、`PARTIAL_SUCCESS`、`FAILED`、`CANCELLED`、`MISSED`。
...
终态：`SUCCEEDED`、`PARTIAL_SUCCESS`、`FAILED`、`CANCELLED`、`MISSED`。
...
| 3 | `cancel_requested=true`，且全部 SubTask 已为 `SUCCESS/FAILED/DLQ/CANCELLED` | `CANCELLED`；结果保留已成功/失败计数 |
| 4 | 全部 SubTask 为 `SUCCESS` | `SUCCEEDED` |
| 5 | 至少一个 `SUCCESS`，且至少一个 `FAILED/DLQ/CANCELLED` | `PARTIAL_SUCCESS` |
| 6 | 无 `SUCCESS`，且至少一个 `FAILED/DLQ`，其余均为 `FAILED/DLQ/CANCELLED` | `FAILED` |
| 7 | 全部 SubTask 为 `CANCELLED` | `CANCELLED` |

`MISSED` 只由尚未开始执行的 `PENDING` Task 产生，不参与 SubTask 聚合。聚合与 Task 状态、结果、预留释放、审计、Outbox 必须同事务提交。

### 9.2 SubTask

状态集：`WAITING_DEPENDENCY`、`WAITING_CAPACITY`、`WAITING_CAPACITY_REPLACEMENT`、`QUEUED`、`ASSIGNED`、`RUNNING`、`WAITING_APPROVAL`、`VERIFYING`、`SUCCESS`、`FAILED`、`DLQ`、`CANCELLED`、`PUBLISHED_UNVERIFIED`。
...
| RUNNING/WAITING_APPROVAL/VERIFYING | `BOUND_DEVICE_PERMANENT_FAILURE`，且完整 WAL 明确证明无副作用 | DLQ |
| RUNNING/VERIFYING | 外部副作用结果不确定 | PUBLISHED_UNVERIFIED |
| PUBLISHED_UNVERIFIED | 人工/自动重新验证成功 | SUCCESS |
| PUBLISHED_UNVERIFIED | 人工确认确定未发布且属于确定业务失败 | FAILED |
...
终态：`SUCCESS`、`FAILED`、`DLQ`、`CANCELLED`。`VERIFYING` 和 `PUBLISHED_UNVERIFIED` 不允许直接取消。

### 9.3 Attempt 与副作用

状态集：`PREPARED`、`INTERACTING`、`WAITING_APPROVAL`、`SUBMITTING`、`VERIFYING`、`SUCCEEDED`、`FAILED`、`ABORTED`、`CANCELLED`、`PUBLISHED_UNVERIFIED`。
...
| PREPARED/INTERACTING/WAITING_APPROVAL | `BOUND_DEVICE_PERMANENT_FAILURE`，且完整 WAL 明确证明无副作用 | FAILED | 禁止；error_category=bound_device_permanent_failure |
...
| PUBLISHED_UNVERIFIED | 只读验证确认已发布 | SUCCEEDED | 不适用 |
| PUBLISHED_UNVERIFIED | 人工确认未发布并终止 | FAILED | 不自动重试 |
...
Attempt 终态：`SUCCEEDED`、`FAILED`、`ABORTED`、`CANCELLED`。`PUBLISHED_UNVERIFIED` 是冻结的人工收敛态，不是终态；它只能经只读验证或人工确认转 `SUCCEEDED/FAILED`，不得启动新副作用。Attempt 收敛驱动 SubTask 的规则：
...
- “放弃整组”只能把所有尚未执行且无副作用的依赖子节点转 `CANCELLED` 并记录人工理由；不确定父节点及其他不确定节点继续保持 `PUBLISHED_UNVERIFIED`，Task 保持 `MANUAL_REVIEW`。

验证必须校验 comment_id、预期账号身份、目标视频和内容指纹，不能只依赖 DOM 文本相似。

依赖传播仅支持以下固定策略；v3.1 不开放客户自定义失败边策略：
...
| `capacity` | 无窗口、无同机替换账号 | WAITING 状态；达到有界时间后 DLQ |
...
- 只读重新验证。
- 满足副作用 guard 时重派。
- 对满足安全 guard 的节点终止或放弃，并填写理由；`PUBLISHED_UNVERIFIED` 不提供强制终态或放弃入口。
...
- `(source_device_id, source_db_uuid, event_id)` 唯一去重。
- source_revision 小于等于 checkpoint：幂等确认，不重复应用。
- source_revision 等于 checkpoint+1：应用并推进 checkpoint。
...
- Central ACK/checkpoint 只表示允许进入保留期清理判断：已 ACK 数据仅在 age >=7 天后可清理；未 ACK 数据无论年龄均不得删除。

### 13.3 保留与背压

| 数据 | 保留策略 |
|---|---|
| Agent 事件/WAL/Receipt | ACK 与否均至少保留 7 天；未 ACK 数据超过 7 天仍不得删除，只能扩容或停领新任务 |
| Central TaskResult/Audit | 热数据 90 天；审计和 DLQ 至少 180 天；之后归档 |
| Central 运行日志 | 30 天；安全审计按合规要求延长 |
| Web 事件流 | 可回放窗口至少 24 小时；容量按压测峰值事件率 × 24 小时 × 1.2 配置，低于 24 小时视为容量验收失败；超窗必须全量补拉 |

Agent 缓冲达到 80% 发告警；达到 95% 停止领取新 Work Unit。禁止删除未确认数据维持运行。
...
- Web 事件连接失败后降级 HTTP 快照/轮询；第 `n` 次连续失败等待 `min(60, 2 × 2^n) + U(0.5, 3)` 秒，连续稳定 60 秒后将 `n` 归零。
...
3. Agent 取得新 recovery_epoch 的 Session 后，只能通过恢复专用摄取接口上传本地保留的旧 epoch 事件/WAL/Receipt。证据保留原 source epoch 和 source_revision，仅进入隔离队列，不直接修改权威状态。
4. 对账应用服务校验原 Task、账号、设备、内容指纹和副作用阶段后才生成恢复决议。恢复点时处于 `SUBMITTING/VERIFYING` 或证据不足的 Attempt 一律转 `PUBLISHED_UNVERIFIED` 并设置 `reassign_blocked=true`，禁止自动补提。
5. 仅当所有在线 Agent checkpoint 连续、离线 Agent 已显式隔离、差异报告归零或进入人工队列后，平台管理员才可 CAS 解除调度冻结。
...
配置修改必须小步灰度并观察失败率、熔断率、DLQ、PUBLISHED_UNVERIFIED 和资源水位。不得直接改库绕过版本化服务。
...
- 策略注册表、CampaignExecutor、Comment Campaign 策略化。
...
| F-10 | M5a | 重复 Inbox event_id | 状态与副作用只应用一次 |
...
| F-14 | M5c | 父节点分别进入 FAILED、DLQ、CANCELLED、PUBLISHED_UNVERIFIED | 依赖节点按第 10 章同事务递归收敛；无永久 WAITING_DEPENDENCY |
...
| F-19 | M5c | 仍有重试次数时，绑定 Device 分别在副作用前、SUBMITTING 后永久故障 | 前者 Attempt=FAILED、活跃 SubTask=DLQ、Task=FAILED 并释放预留，不回 QUEUED；后者 MANUAL_REVIEW 且不重派；两者均不改 bound_device_id |
| F-20 | M5a | ACK 后第 1 天、第 7 天及未 ACK 超过 7 天的 Agent 数据清理 | 第 1 天保留；满 7 天后仅已 ACK 可清理；未 ACK 始终保留 |
...
| F-22 | M5c | 两棵树竞争账号、连续换号并分别进入终态 | 活跃 account_id 不重复；优先消费 RESERVED_SPARE；终态后所有活跃行转 RELEASED |
...
- Central API P50/P95/P99、错误率、连接池水位。
- 调度 tick、Lease 回收、认领冲突、队列深度。
- Agent 在线率、Session Token 失败、SSE 重连、版本分布。
- Profile AVAILABLE/BOUND/BUSY/QUARANTINED/EXTERNAL_BUSY。
- WAITING_WINDOW、WAITING_CAPACITY_REPLACEMENT、DLQ、PUBLISHED_UNVERIFIED。
- grant 创建、领取、过期、重放拒绝。
- 同步 checkpoint 落后、revision 缺口、Agent 缓冲水位。
...
| 单机评论树容量不足 | 创建预检、20% 余量、有界等待、明确 DLQ |
...
| Redis 故障导致事件丢失 | Outbox + HTTP 权威补拉；禁止 Memory 可恢复降级 |
...
本版本无阻塞产品或架构问题。SSO、NATS、跨设备评论树均属于明确非目标或后续独立决策，不得在 M5/M6 实现中自行启用。
...
| Inbox 去重等同动作幂等 | Inbox 只防重复消息；外部动作由 Attempt/WAL 保护 |
