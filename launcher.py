...
from comment_campaign.queueing import RedisLease, RedisUnavailableError
...
class CampaignWorkerCleanupError(RuntimeError):
    """The old Campaign worker could not be proven safe to terminate."""


def _worker_command_matches(command_line: str, identity) -> bool:
    try:
        tokens = [token.strip('"') for token in shlex.split(command_line, posix=False)]
    except ValueError:
        return False
    expected = [
        "-m", "comment_campaign.worker", "serve",
        "--project-fingerprint", identity.project_fingerprint,
        "--owner-nonce", identity.owner_nonce,
    ]
    return any(
...
def _read_windows_process_command_line(pid: int) -> str | None:
    script = (
    ...
    if completed.returncode != 0:
        raise CampaignWorkerCleanupError("无法核对旧评论 Worker 进程。")
...
def _terminate_windows_process(pid: int) -> None:
    ...
    if completed.returncode != 0:
        raise CampaignWorkerCleanupError("无法安全结束旧评论 Worker。")


def stop_same_project_campaign_worker(
    redis_url: str,
    *,
    ...
    sleep=None,
    exit_poll_attempts: int = 20,
) -> int | None:
    """Stop one exact v2 worker or fail closed without broad cleanup."""
    if not isinstance(exit_poll_attempts, int) or exit_poll_attempts <= 0:
        raise ValueError("exit_poll_attempts must be positive")
    if redis_factory is None:
        try:
            import redis
        except ImportError as exc:
            raise CampaignWorkerCleanupError("Redis 不可用，无法核对旧评论 Worker。") from exc
        redis_factory = lambda url: redis.Redis.from_url(
            url, socket_connect_timeout=1.0, socket_timeout=1.0
        )
    reader = process_command_line or _read_windows_process_command_line
    terminator = terminate_process or _terminate_windows_process
    pause = sleep or time.sleep
    try:
        connection = redis_factory(redis_url)
        first_value = connection.get(WORKER_HEALTH_KEY)
    except Exception as exc:
        raise CampaignWorkerCleanupError("Redis 不可用，无法核对旧评论 Worker。") from exc
    if first_value is None:
        return None
    identity = parse_worker_health_value(first_value)
    if identity is None:
        legacy_pid = extract_legacy_worker_pid(first_value)
        if legacy_pid is not None:
            raise CampaignWorkerCleanupError(
                f"旧版评论 Worker 无法自动验证，请人工确认并处理 PID {legacy_pid}。"
            )
        raise CampaignWorkerCleanupError("旧评论 Worker 身份格式无法验证。")
    if identity.project_fingerprint != project_fingerprint(project_root):
        raise CampaignWorkerCleanupError("旧评论 Worker 不属于当前项目。")
    own_pid = os.getpid() if current_pid is None else current_pid
    if identity.pid == own_pid:
        raise CampaignWorkerCleanupError("拒绝结束当前启动器进程。")
    try:
        command_line = reader(identity.pid)
    except Exception as exc:
        raise CampaignWorkerCleanupError("无法核对旧评论 Worker 进程。") from exc
    if command_line is None or not _worker_command_matches(command_line, identity):
        raise CampaignWorkerCleanupError("旧评论 Worker 命令行无法验证。")
    try:
        second_value = connection.get(WORKER_HEALTH_KEY)
    except Exception as exc:
        raise CampaignWorkerCleanupError("Redis 不可用，无法复核旧评论 Worker。") from exc
    if second_value != first_value:
        raise CampaignWorkerCleanupError("旧评论 Worker 租约已变化。")
    try:
        terminator(identity.pid)
    except Exception as exc:
        raise CampaignWorkerCleanupError("无法安全结束旧评论 Worker。") from exc
    for _ in range(exit_poll_attempts):
        try:
            if reader(identity.pid) is None:
                break
        except Exception as exc:
            raise CampaignWorkerCleanupError("无法确认旧评论 Worker 已退出。") from exc
        pause(0.1)
    else:
        raise CampaignWorkerCleanupError("旧评论 Worker 未在限定时间内退出。")
    try:
        owner = first_value.decode("utf-8", "strict") if isinstance(first_value, bytes) else first_value
        released = RedisLease(
            connection, "worker:health", owner, ttl_seconds=30
        ).release()
    except (UnicodeDecodeError, RedisUnavailableError, ValueError) as exc:
        raise CampaignWorkerCleanupError("无法释放旧评论 Worker 租约。") from exc
    if not released:
        raise CampaignWorkerCleanupError("旧评论 Worker 租约已变化。")
    return identity.pid
...
class LauncherApp:
    ...
    def _cleanup_stale_campaign_worker(self, environment: dict[str, str]) -> int | None:
        redis_url = environment.get(
            "COMMENT_CAMPAIGN_REDIS_URL",
            "redis://127.0.0.1:6379/0",
        )
        return stop_same_project_campaign_worker(redis_url)
    ...
    def _restart_services(self, *, run_checks: bool) -> bool:
        ...
        try:
            self._cleanup_stale_campaign_worker(environment)
        except CampaignWorkerCleanupError as error:
