(function (root, factory) {
  ...
  const strategyCore = (typeof module === "object" && module.exports)
    ...
    : root && root.BrowserStrategyEditorCore;
  if (!strategyCore) throw new Error("BrowserStrategyEditorCore 未加载");
  ...
  // Core owns the action labels, including scroll: {label: "切换视频"} for the legacy V2 contract.
  const ACTIONS = strategyCore.ACTIONS;

  function clone(value) { return value === undefined ? undefined : JSON.parse(JSON.stringify(value)); }
  function strategyDraft(record) { return strategyCore.normalizeStrategyDraft(record); }
  ...
  function actionTemplate(type, id) { return strategyCore.actionTemplate(type, id); }
  function newStrategy(id) { return strategyCore.createStrategyDraft(id); }
  ...
  function browserV2Dependencies(win) {
    return {
      ...
      initialView: new URLSearchParams(win.location && win.location.search || "").get("view"),
    };
  }
  ...
  function createBrowserV2UI(dependencies) {
    ...
    const state = {
      view: VIEWS.includes(deps.initialView) ? deps.initialView : "center", profiles: [], profilesAvailable: false, elements: [], contentLibraries: [], strategies: [], history: [], job: null,
    ...
    function eligibleElements(purpose, kinds) {
      return strategyCore.eligibleElements(state.elements, purpose, kinds);
    }
    ...
    function addAction(type) {
      ...
      if (!strategyCore.addAction(state.draft, type, nextActionId(state.draft.definition.actions))) return false;
      renderStrategies();
      return true;
    }
    function moveAction(index, delta) { if (!state.draft || !strategyCore.moveAction(state.draft, index, delta)) return false; renderStrategies(); return true; }
    function removeAction(index) { if (!state.draft || !strategyCore.removeAction(state.draft, index)) return false; renderStrategies(); return true; }
    ...
    function serializeDefinition() {
      const source = state.draft.definition, runMode = el("#v2-strategy-run-mode").value;
      return strategyCore.serializeDefinition({
        target_url: el("#v2-strategy-target-url").value.trim(),
        ready_element_id: el("#v2-strategy-ready-element").value,
        readiness_timeout_seconds: Number(el("#v2-strategy-readiness-timeout").value),
        run_mode: runMode,
        loop_duration_minutes: runMode === "duration" ? el("#v2-strategy-minutes").value : null,
    ...
    async function saveStrategy() {
      ...
      state.draft.name = name; state.draft.definition = definition; state.draft.enabled = el("#v2-strategy-enabled").checked;
      const body = isNew ? strategyCore.buildCreatePayload(state.draft) : strategyCore.buildUpdatePayload(state.draft);
    ...
    function wire() {
      ...
      el("#v2-strategy-copy")?.addEventListener("click", function () { if (!state.draft) return; state.draft = strategyCore.duplicateStrategyDraft(state.draft, "strategy_" + Date.now()); render(); }); el("#v2-strategy-delete")?.addEventListener("click", deleteStrategy);
