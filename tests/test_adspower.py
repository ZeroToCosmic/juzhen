...
from adspower import AdsPowerController, AdsPowerDependencyError, AdsPowerError
...
def test_list_profiles_invalid_raw_list_shape_is_a_safe_dependency_error(monkeypatch):
    monkeypatch.setattr(
        "adspower.requests.get",
        lambda *_args, **_kwargs: SimpleNamespace(
            raise_for_status=lambda: None,
            json=lambda: {"code": 0, "data": {"list": "raw shape SECRET"}},
        ),
    )

    with pytest.raises(AdsPowerDependencyError) as caught:
        AdsPowerController(max_retries=1).list_profiles()

    assert caught.value.reason == "invalid_response"
    assert "SECRET" not in str(caught.value)


def test_list_profiles_missing_list_key_is_a_safe_dependency_error(monkeypatch):
    monkeypatch.setattr(
        "adspower.requests.get",
        lambda *_args, **_kwargs: SimpleNamespace(
            raise_for_status=lambda: None,
            json=lambda: {"code": 0, "data": {"total": 21}},
        ),
    )
...
def test_list_all_profiles_pagination_limit_is_a_safe_dependency_error(monkeypatch):
    controller = AdsPowerController(max_retries=1)
    calls = []
...
def test_business_auth_rejection_uses_a_fixed_safe_reason(monkeypatch):
    response = SimpleNamespace(
        raise_for_status=lambda: None,
        json=lambda: {"code": -1, "msg": "API key invalid SECRET"},
    )
    monkeypatch.setattr("adspower.requests.get", lambda *_args, **_kwargs: response)

    with pytest.raises(AdsPowerDependencyError) as caught:
        AdsPowerController(max_retries=1).list_profiles(page=1, page_size=1)

    assert caught.value.reason == "authentication_failed"
    assert "SECRET" not in str(caught.value)


def _exception_strings(error):
    pending = [error]
    seen: set[int] = set()
    strings = []
    while pending:
...
def test_http_dependency_error_drops_the_entire_sensitive_exception_chain(monkeypatch):
    source = requests.HTTPError(
        "HTTP SECRET",
        response=SimpleNamespace(
            status_code=401,
    ...
    assert "SECRET" not in " ".join(_exception_strings(caught.value))


def test_generic_negative_business_code_is_not_an_authentication_failure(monkeypatch):
    monkeypatch.setattr(
        "adspower.requests.get",
        lambda *_args, **_kwargs: SimpleNamespace(
            raise_for_status=lambda: None,
            json=lambda: {"code": -1, "msg": "profile unavailable SECRET"},
        ),
    )
...
def test_timeout_becomes_a_safe_dependency_error_without_a_chain(monkeypatch):
    monkeypatch.setattr(
        "adspower.requests.get",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(requests.Timeout("SECRET")),
    )
