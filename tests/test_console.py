...
def test_dashboard_opens_settings_by_default():
    ...
    assert '<a class="dashboard-nav-link active" href="/?panel=settings" data-panel="settings" aria-current="page">集中配置</a>' in page
...
def test_dashboard_page_is_primary_workflow_console():
    ...
    assert "自动化主控台" in page
    assert "账号花名册" in page
    assert "代理配置" in page
    assert "发布管理" in page
    assert "浏览器接管" in page
...
def test_dashboard_splits_proxy_config_and_publish_management_pages():
    ...
    assert response.status_code == 200
    page = response.data.decode("utf-8")
    assert 'data-panel="proxy-config"' in page
    assert 'data-panel="publish"' in page
...
def test_dashboard_has_content_management_page():
    ...
    assert response.status_code == 200
    page = response.data.decode("utf-8")
    assert 'data-panel="content"' in page
...
def test_dashboard_links_to_new_stats_page_and_removes_legacy_stats_panel():
    ...
    assert 'href="/tiktok-stats"' in page
    ...
    assert 'data-panel="publish-results"' in page
...
def test_dashboard_uses_shared_expanded_sidebar_with_real_panel_urls():
    ...
    expected_links = [
        ('/?panel=settings', 'data-panel="settings"'),
        ('/?panel=accounts', 'data-panel="accounts"'),
        ('/?panel=proxy-config', 'data-panel="proxy-config"'),
        ('/?panel=content', 'data-panel="content"'),
        ('/?panel=publish', 'data-panel="publish"'),
        ('/?panel=publish-results', 'data-panel="publish-results"'),
        ('/tiktok-stats', 'aria-label="数据统计"'),
        ('/?panel=browser', 'data-panel="browser"'),
        ('/?panel=strategies', 'data-panel="strategies"'),
    ]
    positions = []
    ...
    assert 'href="/?panel=settings"' in page
