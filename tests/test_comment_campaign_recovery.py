...
class PrepareOnlyQueue:
    ...
    def enqueue_prepare_generation(self, campaign_id: str, generation: int):
        self.calls.append(("prepare", campaign_id, generation))
...
def _awaiting_with_consumed_approval(service, assignment_id: str):
...
def test_reconcile_consumed_approval_reprepares_with_new_revision_never_submit(tmp_path):
    ...
    assert queue.calls == [("prepare", "recovery-campaign", 2)]
...
def test_reconcile_marks_interrupted_submit_unverified_and_never_replays_it(tmp_path, interrupted_status):
    service, queue, root = _runtime(tmp_path)
    awaiting = _awaiting_with_consumed_approval(service, root["assignment_id"])
    submitting = service.store.begin_submitting("recovery-campaign", root["assignment_id"], awaiting["revision"])
...
def test_reconcile_rolls_back_receipt_attempt_and_status_when_recovery_audit_fails(tmp_path, monkeypatch):
    ...
    awaiting = _awaiting_with_consumed_approval(service, root["assignment_id"])
    service.store.begin_submitting("recovery-campaign", root["assignment_id"], awaiting["revision"])
...
def test_threaded_recovery_pauses_only_descendants_and_rolls_back_pause_fault(tmp_path, monkeypatch):
    ...
    plan=service.plan_campaign("tree-c", seed="tree"); locked=service.lock_plan("tree-c",plan["campaign"]["revision"]); campaign=service.approve_campaign("tree-c",locked["revision"])["campaign"]; store.transition_campaign_status("tree-c",campaign["revision"],"running")
    ...
    store.create_submit_approval("tree-c",root_child["assignment_id"],root_child["revision"],"x"); store.consume_submit_approval("tree-c",root_child["assignment_id"],root_child["revision"]); store.begin_submitting("tree-c",root_child["assignment_id"],root_child["revision"])
    service.reconcile_campaign("tree-c")
    states={row["step_id"]:row["status"] for row in store.list_assignments("tree-c")}
    assert states["root"] == "published_verified" and states["child"] == "published_unverified" and states["grand"] == "paused_dependency" and states["sibling"] == "planned"
