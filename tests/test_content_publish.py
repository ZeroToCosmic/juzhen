...
def test_publish_results_supports_bounded_server_pagination_and_search(monkeypatch, tmp_path):
    client, data_dir, _db_path = make_client(tmp_path, monkeypatch)
    write_json(
        data_dir / "publish_tasks.json",
        {
            "tasks": [
                {"id": "task-1", "status": "success", "account_id": "alpha", "created_at": "2026-07-10T08:00:00+08:00"},
                {"id": "task-2", "status": "failed", "account_id": "beta", "error": "Buffer timeout", "created_at": "2026-07-10T09:00:00+08:00"},
                {"id": "task-3", "status": "pending", "account_id": "gamma", "created_at": "2026-07-10T10:00:00+08:00"},
            ]
        },
    )

    page = client.get("/api/publish/results?page=1&page_size=2")
    search = client.get("/api/publish/results?query=buffer&page=1&page_size=50")

    assert page.status_code == 200
    assert page.get_json()["total"] == 3
    assert page.get_json()["total_pages"] == 2
    assert len(page.get_json()["tasks"]) == 2
    assert page.get_json()["summary"] == {"task_count": 3, "success": 1, "failed": 1, "pending": 1}
    assert [item["id"] for item in search.get_json()["tasks"]] == ["task-2"]
