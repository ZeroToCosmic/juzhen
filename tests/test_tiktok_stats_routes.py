...
def route_app(tmp_path: Path):
    ...
    with StatsStore(stats_path) as store:
        ...
        store.upsert_recent_posts(
            alpha["id"],
            [{
                "video_id": "route-video-1",
                "created_at": "2026-07-21T02:00:00Z",
                "description": "Route video",
                "view_count": 1200,
                "like_count": 120,
                "comment_count": 24,
                "share_count": 3,
...
def test_page_and_get_accounts_are_read_only_and_close_query_services(route_app, client):
    ...
    assert 'href="/console/collection-results"' in page_text
    stats_href = page_text.index('href="/console/collection-results"')
    ...
    expected_hrefs = [
        ...
        '/console/publishing',
        '/console/collection',
        '/console/collection-results',
        '/console/accounts-windows',
...
def test_video_results_route_exposes_only_business_fields_and_closes_query(client):
    response = client.get(
        "/api/tiktok-stats/videos?query=route&account_id=1&sort=views&direction=desc&page=1&page_size=50"
    )
    ...
    assert set(payload["rows"][0]) == {
        "video_id", "description", "account_id", "username", "published_at",
        "views", "likes", "comments", "last_collected_at",
    }
...
def test_video_results_route_rejects_invalid_queries(client, query):
    response = client.get(f"/api/tiktok-stats/videos?{query}")
