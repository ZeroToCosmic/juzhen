...
from launcher import (
    ...
    show_startup_error,
    stop_same_project_campaign_worker,
    stop_port_listeners,
)
from comment_campaign.worker import WORKER_HEALTH_KEY
...
def test_comment_campaign_worker_supervisor_starts_hidden_worker(tmp_path):
    ...
    command = calls[0][0]
    assert command[1:4] == ["-m", "comment_campaign.worker", "serve"]
    assert command[4:6] == [
        "--project-fingerprint",
        project_fingerprint(launcher_module.PROJECT_ROOT),
    ]
    assert command[6] == "--owner-nonce"
    assert parse_worker_health_value(
        build_worker_health_value(1, launcher_module.PROJECT_ROOT, command[7])
    ) is not None
...
class WorkerHealthRedis:
    def __init__(self, value, *, second_value=None, fail_get=False):
        self.current = value
        self.second_value = value if second_value is None else second_value
        self.fail_get = fail_get
        self.get_calls = 0
        self.eval_calls = []

    def get(self, key):
        assert key == WORKER_HEALTH_KEY
        if self.fail_get:
            raise OSError("redis unavailable")
        self.get_calls += 1
        if self.get_calls == 2:
            self.current = self.second_value
        return self.current

    def eval(self, script, keys, key, owner, *_args):
        self.eval_calls.append((script, keys, key, owner))
...
def _worker_command(identity):
    return (
        "python -m comment_campaign.worker serve "
        f"--project-fingerprint {identity.project_fingerprint} "
        f"--owner-nonce {identity.owner_nonce}"
    )


def test_safe_worker_cleanup_terminates_exact_same_project_owner():
    owner = build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "a" * 32)
    identity = parse_worker_health_value(owner)
    redis = WorkerHealthRedis(owner)
    alive = {"value": True}
    terminated = []

    stopped = stop_same_project_campaign_worker(
        "redis://test",
        redis_factory=lambda _url: redis,
        process_command_line=lambda _pid: (
            _worker_command(identity) if alive["value"] else None
        ),
        terminate_process=lambda pid: (
            terminated.append(pid), alive.update(value=False)
        ),
        current_pid=9999,
        sleep=lambda _seconds: None,
    )
    ...
    assert redis.current is None
    assert redis.eval_calls[0][2:] == (WORKER_HEALTH_KEY, owner)


@pytest.mark.parametrize(
    "owner,current_pid",
    [
        ("worker:8123:" + "a" * 32, 9999),
        ("worker:8123:legacy", 9999),
        ("malformed", 9999),
        (build_worker_health_value(8123, Path("foreign-project"), "b" * 32), 9999),
        (build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "c" * 32), 8123),
    ],
)
def test_safe_worker_cleanup_refuses_unverifiable_owners(owner, current_pid):
    redis = WorkerHealthRedis(owner)
    terminated = []

    with pytest.raises(CampaignWorkerCleanupError) as captured:
        stop_same_project_campaign_worker(
            "redis://test",
            redis_factory=lambda _url: redis,
            process_command_line=lambda _pid: "must not be used",
            terminate_process=terminated.append,
            current_pid=current_pid,
        )

    assert terminated == []
    assert redis.current == owner
    assert redis.eval_calls == []
    if owner == "worker:8123:" + "a" * 32:
        assert "PID 8123" in str(captured.value)


def test_safe_worker_cleanup_refuses_missing_or_reused_pid():
    owner = build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "d" * 32)
    redis = WorkerHealthRedis(owner)
    ...
    with pytest.raises(RuntimeError):
        stop_same_project_campaign_worker(
            "redis://test",
            redis_factory=lambda _url: redis,
            process_command_line=lambda _pid: None,
            terminate_process=lambda _pid: pytest.fail("must not terminate"),
            current_pid=9999,
        )
    ...
    with pytest.raises(RuntimeError):
        stop_same_project_campaign_worker(
            "redis://test",
            redis_factory=lambda _url: WorkerHealthRedis(owner),
            process_command_line=lambda _pid: _worker_command(identity).replace(
                identity.owner_nonce, "e" * 32
            ),
            terminate_process=lambda _pid: pytest.fail("must not terminate"),
            current_pid=9999,
        )
...
def test_safe_worker_cleanup_refuses_lease_change_and_redis_failure():
    owner = build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "f" * 32)
    changed = build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "1" * 32)
    identity = parse_worker_health_value(owner)
    terminated = []
    ...
    with pytest.raises(RuntimeError):
        stop_same_project_campaign_worker(
            "redis://test",
            redis_factory=lambda _url: WorkerHealthRedis(owner, second_value=changed),
            process_command_line=lambda _pid: _worker_command(identity),
            terminate_process=terminated.append,
            current_pid=9999,
        )
    assert terminated == []
    ...
    with pytest.raises(RuntimeError):
        stop_same_project_campaign_worker(
            "redis://test",
            redis_factory=lambda _url: WorkerHealthRedis(owner, fail_get=True),
            process_command_line=lambda _pid: pytest.fail("must not inspect"),
            terminate_process=terminated.append,
            current_pid=9999,
        )
    assert terminated == []
...
def test_safe_worker_cleanup_keeps_lease_when_termination_or_cas_fails():
    owner = build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "2" * 32)
    identity = parse_worker_health_value(owner)
    redis = WorkerHealthRedis(owner)
    ...
    with pytest.raises(RuntimeError):
        stop_same_project_campaign_worker(
            ...
            redis_factory=lambda _url: redis,
            process_command_line=lambda _pid: _worker_command(identity),
            terminate_process=lambda _pid: (_ for _ in ()).throw(OSError("denied")),
            current_pid=9999,
        )
    assert redis.current == owner
    assert redis.eval_calls == []
    ...
    alive = {"value": True}
    raced = WorkerHealthRedis(owner)

    def terminate_with_race(_pid):
        alive["value"] = False
        raced.current = build_worker_health_value(
            8123, launcher_module.PROJECT_ROOT, "3" * 32
        )

    with pytest.raises(RuntimeError):
        stop_same_project_campaign_worker(
            "redis://test",
            redis_factory=lambda _url: raced,
            process_command_line=lambda _pid: (
                _worker_command(identity) if alive["value"] else None
            ),
            terminate_process=terminate_with_race,
            current_pid=9999,
            sleep=lambda _seconds: None,
        )
    assert raced.current != owner


def test_safe_worker_cleanup_requires_bounded_process_exit():
    owner = build_worker_health_value(8123, launcher_module.PROJECT_ROOT, "4" * 32)
    identity = parse_worker_health_value(owner)
    redis = WorkerHealthRedis(owner)
...
def launcher_for_restart(events):
    ...
    launcher._cleanup_stale_campaign_worker = lambda _environment: None
...
def test_restart_cleans_stale_campaign_worker_before_starting_any_service(monkeypatch):
    events = []
    launcher = launcher_for_restart(events)
    launcher._cleanup_stale_campaign_worker = (
        lambda _environment: events.append("campaign-cleanup")
    )
    monkeypatch.setattr(
        "launcher.stop_port_listeners",
        lambda port: events.append(f"port-stop:{port}"),
    ...
    assert launcher._restart_services(run_checks=False) is True
    assert events.index("campaign-cleanup") < events.index("flask-start")


def test_restart_fails_closed_when_stale_campaign_cleanup_fails(monkeypatch):
    events = []
    launcher = launcher_for_restart(events)
    launcher._cleanup_stale_campaign_worker = lambda _environment: (
        (_ for _ in ()).throw(CampaignWorkerCleanupError("unsafe worker owner"))
    )
    monkeypatch.setattr(
        "launcher.stop_port_listeners",
        lambda port: events.append(f"port-stop:{port}"),
