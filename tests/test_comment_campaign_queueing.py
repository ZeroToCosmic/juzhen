...
def test_prepare_job_carries_only_safe_ids_and_two_generations():
    queue = FakeQueue()
    coordinator = QueueCoordinator(queue, redis=FakeRedis())

    first = coordinator.enqueue_prepare_generation("campaign-1", 7, 3)
    second = coordinator.enqueue_prepare_generation("campaign-1", 7, 3)

    assert second is first
    assert len(queue.calls) == 1
    assert queue.calls[0] == (
        "enqueue", "comment_campaign.jobs.run_prepare_campaign", ("campaign-1", 7, 3),
        {"job_id": "campaign-prepare-campaign-1-g7", "job_timeout": 600, "result_ttl": 86400},
    )
    assert not hasattr(coordinator, "enqueue_prepare")
...
def test_enqueue_at_is_idempotent_and_passes_utc_datetime_and_both_generations():
    queue = FakeQueue()
    coordinator = QueueCoordinator(queue, redis=FakeRedis())
    when = datetime(2026, 8, 7, 8, tzinfo=timezone.utc)

    first = coordinator.enqueue_at("campaign-1", when, 7, 3)
    second = coordinator.enqueue_at("campaign-1", when, 7, 3)

    assert second is first
    assert len(queue.calls) == 1
    assert queue.calls[0][0] == "enqueue_at"
    assert queue.calls[0][1] == when
    assert queue.calls[0][3] == ("campaign-1", 7, 3)
    assert queue.calls[0][4]["job_id"] == "campaign-prepare-campaign-1-g7"
...
def test_concurrent_queue_gate_never_calls_enqueue_twice_before_job_is_visible():
    ...
    gate = RedisLease(redis, "job:campaign-prepare-campaign-1-g7", "other", ttl_seconds=30)
    assert gate.acquire() is True

    with pytest.raises(RuntimeError, match="retry shortly"):
        contender.enqueue_prepare_generation("campaign-1", 7, 3)
    assert queue.calls == []
...
def test_enqueue_failure_releases_only_its_own_gate():
    ...
    with pytest.raises(OSError, match="queue down"):
        QueueCoordinator(FailingQueue(), redis=redis).enqueue_prepare_generation("campaign-1", 7, 3)
    assert PREFIX + "job:campaign-prepare-campaign-1-g7" not in redis.values
